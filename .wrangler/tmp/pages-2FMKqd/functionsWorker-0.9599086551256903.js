...
async function onRequest(context) {
  ...
  if (path === "/a185616fac3e0.js" || path.startsWith("/f2d4c400adb21")) {
    return next();
  }
  const resp = await next();
  const ctype = resp.headers.get("content-type") || "";
  if (!ctype.includes("text/html") || resp.status !== 200) {
    return resp;
  }
  const INJECT_HTML = `
  ...
  const rewriter = new HTMLRewriter().on("body", {
    element(el) {
      el.append(INJECT_HTML, { html: true });
    }
  });
...
// ../.wrangler/tmp/pages-2FMKqd/functionsRoutes-0.8971438719732767.mjs
var routes = [
  {
    routePath: "/",
    mountPath: "/",
